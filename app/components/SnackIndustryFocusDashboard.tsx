...
function ResearchProductionGate({ locale, design, run, onBackToExecution }: { locale: Locale; design: LockedProjectDesign | null; run: ProjectRunRecord | null; onBackToExecution: () => void }) {
  ...
  return <div className="snack-stack research-production-flow">
    ...
    <section className="execution-design-gate production-step-gate"><span>PROJECT RUN REQUIRED</span><h3>{reason}</h3><p>{tr(locale, "先在第03步完成程序配置、DE小流量检查、正式回收和配额关闭，再选择当前回收数据或上传Raw Data进入生产。", "Complete programming, DE soft launch, fieldwork and quota closure in step 03, then choose the current collection or upload Raw Data for production.")}</p><button type="button" onClick={onBackToExecution}>{tr(locale, "返回第03步", "Return to step 03")}</button></section>
  </div>;
}
...
function ProjectExecutionHub({ locale, design, run, syncState, onBackToDesign, onRunChange }: { locale: Locale; design: LockedProjectDesign | null; run: ProjectRunRecord | null; syncState: "idle" | "saving" | "saved" | "error"; onBackToDesign: () => void; onRunChange: (run: ProjectRunRecord) => void }) {
  ...
  const [frmUrl, setFrmUrl] = useState(defaultFrmUrl);
  ...
  useEffect(() => {
    ...
    setRandomSeed(activeRun.softLaunch?.randomSeed ?? `DE-${activeRun.designVersion}-01`);
    ...
    setFrmUrl(activeRun.fieldwork.frmUrl ?? defaultFrmUrl);
  ...
  const saveProgramming = () => {
    if (!activeRun) return;
    const hasProgram = Boolean(programId.trim() || accessUrl.trim());
    if (hasProgram && (!programId.trim() || !/^https?:\/\//i.test(accessUrl.trim()))) return;
    const now = new Date().toISOString();
    onRunChange({ ...activeRun, stage: "soft_launch", updatedAt: now, program: hasProgram ? { programId: programId.trim(), accessUrl: accessUrl.trim(), confirmedAt: now } : undefined });
  };
  const saveSoftLaunch = () => {
    const n = Number(softLaunchN);
    if (!activeRun || n < 30 || !randomSeed.trim() || !Object.values(softChecks).every(Boolean)) return;
    const now = new Date().toISOString();
  ...
  const saveFieldwork = (close = false) => {
    ...
    const now = new Date().toISOString();
    onRunChange({ ...activeRun, stage: close ? "closed" : "fieldwork", updatedAt: now, closedAt: close ? now : activeRun.closedAt, fieldwork: { completedN: complete, minimumQuotaCompletion: quotaCompletion, frmUrl: /^https?:\/\//i.test(frmUrl.trim()) ? frmUrl.trim() : activeRun.fieldwork.frmUrl, updatedAt: now } });
  };
  ...
  const stages = [
    ...
    { code: "02", zh: "DE小流量检查", en: "DE soft launch", done: softDone, active: activeRun?.stage === "soft_launch", detailZh: "用固定随机数检查路由、随机组、字段和配额计数", detailEn: "Use a fixed random seed to check routing, random cells, fields and quota counts" },
  ...
  const stageLabel = !activeRun ? tr(locale, "未建立", "Not created") : activeRun.stage === "programming" ? tr(locale, "程序配置", "Programming") : activeRun.stage === "soft_launch" ? tr(locale, "DE小流量检查", "DE soft launch") : activeRun.stage === "fieldwork" ? tr(locale, "正式回收", "Fieldwork") : activeRun.stage === "closed" ? tr(locale, "等待数据确认", "Waiting for data confirmation") : tr(locale, "生产数据已绑定", "Production data bound");
  ...
  return <div className="snack-stack project-execution-hub client-progress-hub">
    ...
    {activeRun ? <section className="execution-stage-control">
      {activeRun.stage === "programming" ? <><header><span>01 · PROGRAMMING</span><h3>{tr(locale, "登记程序版本与测试访问入口", "Register the program version and test URL")}</h3></header><div className="execution-control-fields"><label><span>{tr(locale, "程序版本 / ID", "Program version / ID")}</span><input value={programId} onChange={(event) => setProgramId(event.target.value)} placeholder="PROGRAM-V2-R1" /></label><label><span>{tr(locale, "测试访问链接", "Test access URL")}</span><input value={accessUrl} onChange={(event) => setAccessUrl(event.target.value)} placeholder="https://..." /></label></div><footer><p>{tr(locale, "当前案例允许暂不登记；正式执行时版本与链接需同时有效。", "This case can proceed without them; production fieldwork requires a valid version and URL together.")}</p><button type="button" disabled={Boolean(programId.trim() || accessUrl.trim()) && (!programId.trim() || !/^https?:\/\//i.test(accessUrl.trim()))} onClick={saveProgramming}>{tr(locale, "进入DE检查", "Continue to DE check")}</button></footer></> : null}
      {activeRun.stage === "soft_launch" ? <><header><span>02 · DE SOFT LAUNCH</span><h3>{tr(locale, "登记随机数与小流量检查结果", "Register the random seed and DE checks")}</h3></header><div className="execution-control-fields"><label><span>{tr(locale, "DE随机数", "DE random seed")}</span><input value={randomSeed} onChange={(event) => setRandomSeed(event.target.value)} placeholder={`DE-${activeRun.designVersion}-01`} /></label><label><span>{tr(locale, "有效完成样本", "Valid completes")}</span><input type="number" min="0" value={softLaunchN} onChange={(event) => setSoftLaunchN(event.target.value)} placeholder="30" /></label></div><div className="soft-launch-control"><div>{([{ key: "routingPassed", zh: "跳转与Base", en: "Routing and bases" }, { key: "randomizationPassed", zh: "随机分组平衡", en: "Random-cell balance" }, { key: "fieldMapPassed", zh: "Raw字段映射", en: "Raw field mapping" }, { key: "quotaCountPassed", zh: "配额计数", en: "Quota counts" }] as const).map((item) => <label key={item.key} className={softChecks[item.key] ? "checked" : ""}><input type="checkbox" checked={softChecks[item.key]} onChange={(event) => setSoftChecks((current) => ({ ...current, [item.key]: event.target.checked }))} /><span>{tr(locale, item.zh, item.en)}</span></label>)}</div></div><footer><p>{tr(locale, "固定随机数、N≥30且四项全部通过后进入正式回收。", "A fixed seed, N≥30 and all four checks are required before main fieldwork.")}</p><button type="button" disabled={Number(softLaunchN) < 30 || !randomSeed.trim() || !Object.values(softChecks).every(Boolean)} onClick={saveSoftLaunch}>{tr(locale, "确认DE检查", "Confirm DE check")}</button></footer></> : null}
      {activeRun.stage === "fieldwork" ? <><header><span>03–04 · FIELDWORK</span><h3>{tr(locale, "正式回收与FRM配额监控", "Main fieldwork and FRM quota monitoring")}</h3></header><div className="execution-control-fields"><label><span>{tr(locale, "当前有效完成样本", "Current valid completes")}</span><input type="number" min="0" max={activeRun.targetN} value={completedN} onChange={(event) => setCompletedN(event.target.value)} /></label><label><span>{tr(locale, "最低配额单元完成率", "Minimum quota-cell completion")}</span><input type="number" min="0" max="100" value={minimumQuotaCompletion} onChange={(event) => setMinimumQuotaCompletion(event.target.value)} /></label><label className="execution-frm-field"><span>FRM {tr(locale, "监控链接", "monitoring URL")}</span><input value={frmUrl} onChange={(event) => setFrmUrl(event.target.value)} /></label><a className="execution-frm-link" href={/^https?:\/\//i.test(frmUrl.trim()) ? frmUrl.trim() : defaultFrmUrl} target="_blank" rel="noreferrer">{tr(locale, "打开FRM配额监控", "Open FRM quota monitor")} ↗</a></div><footer><p>{tr(locale, `关闭条件：有效完成N=${activeRun.targetN.toLocaleString()}且最低配额单元达到100%。`, `Close when valid completes reach N=${activeRun.targetN.toLocaleString()} and the minimum quota cell reaches 100%.`)}</p><div><button type="button" onClick={() => saveFieldwork(false)}>{tr(locale, "保存进度", "Save progress")}</button><button type="button" className="primary" disabled={Number(completedN) < activeRun.targetN || Number(minimumQuotaCompletion) < 100} onClick={() => saveFieldwork(true)}>{tr(locale, "关闭回收", "Close fieldwork")}</button></div></footer></> : null}
      {activeRun.stage === "closed" ? <div className="execution-ready-for-data"><span>05 · FIELDWORK CLOSED</span><h3>{tr(locale, "回收已关闭，进入第04步确认生产数据", "Fieldwork is closed; confirm production data in step 04")}</h3><p>{tr(locale, "可直接使用当前回收结果，或上传另一份已确认Raw Data。", "Use the current collection or upload another confirmed Raw Data file.")}</p></div> : null}
