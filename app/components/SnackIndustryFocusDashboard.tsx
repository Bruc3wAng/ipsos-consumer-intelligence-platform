...
import { checkSetReady, deCheckReady, fieldworkCloseReady, programConfigurationReady } from "../lib/projectExecutionWorkflow";
...
function ResearchProductionGate({ locale, design, run, onBackToExecution }: { locale: Locale; design: LockedProjectDesign | null; run: ProjectRunRecord | null; onBackToExecution: () => void }) {
  ...
  return <div className="snack-stack research-production-flow">
    <SectionTitle eyebrow="DATA → TABLE → MODEL" title={tr(locale, "从确认数据到Table、指标与模型", "From confirmed data to tables, metrics and models")} note={tr(locale, "第04步只接收已关闭回收且与当前研究设计一致的项目运行记录。", "Step 04 only accepts a closed project run that matches the current research design.")} />
    <section className="execution-design-gate production-step-gate"><span>PROJECT RUN REQUIRED</span><h3>{reason}</h3><p>{tr(locale, "先在第03步依次完成程序配置、程序测试、DE随机数检查、正式上线、配额监控和回收关闭，再选择当前回收数据或上传Raw Data进入生产。", "Complete programming, program testing, DE random-seed checks, launch, quota monitoring and fieldwork closure in step 03 before selecting the current collection or uploading Raw Data.")}</p><button type="button" onClick={onBackToExecution}>{tr(locale, "返回第03步", "Return to step 03")}</button></section>
  </div>;
}

function ResearchProductionFlow({ locale, design, run, initialProduction, onProduction, onFinalRawBound }: { locale: Locale; design: LockedProjectDesign; run: ProjectRunRecord; initialProduction: RawProductionResult | null; onProduction: (result: RawProductionResult) => void; onFinalRawBound: (result: RawProductionResult, source: "current_collection" | "uploaded_raw") => void }) {
  ...
  const [dataSource, setDataSource] = useState<"current_collection" | "uploaded_raw">(run.dataConfirmation?.source ?? "current_collection");

  const runProduction = useCallback(async (file: File, source: "current_collection" | "uploaded_raw" = "uploaded_raw") => {
    ...
    try {
      ...
      if (payload.meta.status === "ready") onFinalRawBound(payload, source);
  ...
  const loadCurrentCollection = useCallback(async () => {
    setDataSource("current_collection");
    setUploadState("processing");
    setUploadMessage(tr(locale, "正在载入本项目回收数据", "Loading the current project collection"));
    try {
      const response = await fetch(publicAssetPath("/downloads/中国薄脆饼干新品概念与定价研究_FINAL-R1_Raw.csv"));
      if (!response.ok) throw new Error(tr(locale, "当前回收数据暂时无法读取", "The current collection is unavailable"));
      const file = new File([await response.blob()], "中国薄脆饼干新品概念与定价研究_FINAL-R1_Raw.csv", { type: "text/csv;charset=utf-8" });
      await runProduction(file, "current_collection");
    } catch (error) {
      setUploadState("error");
      setUploadMessage(error instanceof Error ? error.message : tr(locale, "当前回收数据读取失败", "Current collection loading failed"));
    }
  }, [locale, runProduction]);
  ...
  return <div className="snack-stack research-production-flow">
    <SectionTitle eyebrow="DATA → TABLE → MODEL" title={tr(locale, "从确认数据到Table、指标与模型", "From confirmed data to tables, metrics and models")} note={tr(locale, "本页只接收已确认进入分析的数据，并按第02步锁定的题号、Base、权重与Banner生产结果。", "This page accepts confirmed analysis data and uses the question IDs, bases, weights and banners locked in step 02.")} />
    <section className="production-output-chain data-production-chain"><article><span>01</span><b>{tr(locale, "确认数据版本", "Confirm data version")}</b><p>{tr(locale, "当前回收或上传Raw", "Current collection or uploaded Raw")}</p></article><i>→</i><article><span>02</span><b>{tr(locale, "字段与版本对齐", "Align fields and version")}</b><p>{tr(locale, "题号、Code、Base、权重", "IDs, codes, bases and weights")}</p></article><i>→</i><article><span>03</span><b>Table</b><p>Count · No sig · Sig</p></article><i>→</i><article><span>04</span><b>KPI</b><p>{tr(locale, "通用与项目专项指标", "Common and project KPIs")}</p></article><i>→</i><article><span>05</span><b>{tr(locale, "模型运行与验证", "Model run and validation")}</b><p>{tr(locale, "驱动、选择与预测证据", "Driver, choice and forecast evidence")}</p></article></section>

    <section className="final-data-intake">
      <header><div><span>DATA CONFIRMATION</span><h3>{tr(locale, "选择进入Table与模型的数据", "Choose the data used for tables and models")}</h3><p>{tr(locale, "两种入口使用同一字段契约与同一生产逻辑；每次确认都会形成独立结果版本。", "Both routes use the same field contract and production logic; each confirmation creates a separate result version.")}</p></div><div className="final-data-actions data-source-actions"><button type="button" className={dataSource === "current_collection" ? "active" : ""} onClick={() => void loadCurrentCollection()}>{tr(locale, "使用当前回收数据", "Use current collection")}</button><label className={`final-data-upload ${dataSource === "uploaded_raw" ? "active" : ""}`}><input type="file" accept=".csv,text/csv" onChange={(event) => { const file = event.target.files?.[0]; if (file) { setDataSource("uploaded_raw"); void runProduction(file, "uploaded_raw"); } }} /><span>{tr(locale, "上传Raw Data", "Upload Raw Data")}</span></label></div></header>
...
function ProjectExecutionHub({ locale, design, run, syncState, onBackToDesign, onRunChange }: { locale: Locale; design: LockedProjectDesign | null; run: ProjectRunRecord | null; syncState: "idle" | "saving" | "saved" | "error"; onBackToDesign: () => void; onRunChange: (run: ProjectRunRecord) => void }) {
  const defaultFrmUrl = "https://rep751.ipsosinteractive.com/FRMv75/ClientPages/Main.aspx?cHJvamVjdElEPTI2MDczJmRlbW9saW5rPTEmdD02MzkyMjMyNDUwNjY2MzM2Nj";
  const defaultProgramId = "PROGRAM-V2-R1";
  const fixedTestPath = publicAssetPath("/research-operations/snack/program-test/");
  const activeRun = design && run?.designVersion === design.designVersion ? run : null;
  const [programId, setProgramId] = useState(activeRun?.program?.programId ?? defaultProgramId);
  const [accessUrl, setAccessUrl] = useState(activeRun?.program?.accessUrl ?? fixedTestPath);
  const [programChecks, setProgramChecks] = useState(activeRun?.programTest ? { questionnaireLoaded: activeRun.programTest.questionnaireLoaded, routingPassed: activeRun.programTest.routingPassed, validationPassed: activeRun.programTest.validationPassed, completionPassed: activeRun.programTest.completionPassed } : { questionnaireLoaded: false, routingPassed: false, validationPassed: false, completionPassed: false });
  const [softLaunchN, setSoftLaunchN] = useState(activeRun?.softLaunch ? String(activeRun.softLaunch.completedN) : "");
  const [randomSeed, setRandomSeed] = useState(activeRun?.softLaunch?.randomSeed ?? (activeRun ? `DE-${activeRun.designVersion}-01` : ""));
  const [softChecks, setSoftChecks] = useState(activeRun?.softLaunch ? { routingPassed: activeRun.softLaunch.routingPassed, randomizationPassed: activeRun.softLaunch.randomizationPassed, fieldMapPassed: activeRun.softLaunch.fieldMapPassed, quotaCountPassed: activeRun.softLaunch.quotaCountPassed } : { routingPassed: false, randomizationPassed: false, fieldMapPassed: false, quotaCountPassed: false });
  const [completedN, setCompletedN] = useState(String(activeRun?.fieldwork.completedN ?? 0));
  const [minimumQuotaCompletion, setMinimumQuotaCompletion] = useState(String(activeRun?.fieldwork.minimumQuotaCompletion ?? 0));
  const [frmUrl, setFrmUrl] = useState(activeRun?.fieldwork.frmUrl ?? defaultFrmUrl);
  ...
  const saveProgramming = () => {
    if (!activeRun) return;
    if (!programConfigurationReady(programId, accessUrl)) return;
    const now = new Date().toISOString();
    onRunChange({ ...activeRun, stage: "program_test", updatedAt: now, program: { programId: programId.trim(), accessUrl: accessUrl.trim(), confirmedAt: now } });
  };
  const saveProgramTest = () => {
    if (!activeRun || !activeRun.program || !checkSetReady(programChecks)) return;
    const now = new Date().toISOString();
    onRunChange({ ...activeRun, stage: "soft_launch", updatedAt: now, programTest: { ...programChecks, confirmedAt: now } });
  };
  const saveSoftLaunch = () => {
    const n = Number(softLaunchN);
    if (!activeRun || !deCheckReady(n, randomSeed, softChecks)) return;
    const now = new Date().toISOString();
    onRunChange({ ...activeRun, stage: "fieldwork", updatedAt: now, softLaunch: { completedN: n, randomSeed: randomSeed.trim(), ...softChecks, confirmedAt: now }, fieldwork: { ...activeRun.fieldwork, frmUrl: /^https?:\/\//i.test(frmUrl.trim()) ? frmUrl.trim() : defaultFrmUrl } });
  };
  const confirmFieldworkLaunch = () => {
    if (!activeRun || !activeRun.programTest || !activeRun.softLaunch) return;
    const now = new Date().toISOString();
    onRunChange({ ...activeRun, stage: "monitoring", updatedAt: now, fieldwork: { ...activeRun.fieldwork, frmUrl: /^https?:\/\//i.test(frmUrl.trim()) ? frmUrl.trim() : defaultFrmUrl, startedAt: now, updatedAt: now } });
  };
  const saveFieldwork = (close = false) => {
    if (!activeRun || activeRun.stage !== "monitoring") return;
    ...
    if (close && !fieldworkCloseReady(complete, activeRun.targetN, quotaCompletion)) return;
    const now = new Date().toISOString();
    onRunChange({ ...activeRun, stage: close ? "closed" : "monitoring", updatedAt: now, closedAt: close ? now : activeRun.closedAt, fieldwork: { ...activeRun.fieldwork, completedN: complete, minimumQuotaCompletion: quotaCompletion, frmUrl: /^https?:\/\//i.test(frmUrl.trim()) ? frmUrl.trim() : activeRun.fieldwork.frmUrl, updatedAt: now } });
  };
  const resetRun = () => {
    if (!activeRun) return;
    const now = new Date().toISOString();
    setProgramId(defaultProgramId);
    setAccessUrl(fixedTestPath);
    setProgramChecks({ questionnaireLoaded: false, routingPassed: false, validationPassed: false, completionPassed: false });
    setSoftLaunchN("");
    setRandomSeed(`DE-${activeRun.designVersion}-01`);
    ...
    onRunChange({ runId: activeRun.runId, projectId: activeRun.projectId, designVersion: activeRun.designVersion, designConfirmationKey: activeRun.designConfirmationKey, designSnapshot: activeRun.designSnapshot, stage: "programming", createdAt: activeRun.createdAt, updatedAt: now, targetN: activeRun.targetN, fieldwork: { completedN: 0, minimumQuotaCompletion: 0, frmUrl: defaultFrmUrl, updatedAt: null } });
  };
  const programDone = Boolean(activeRun && activeRun.stage !== "programming");
  const programTestDone = Boolean(activeRun?.programTest);
  const softDone = Boolean(activeRun?.softLaunch);
  const fieldworkStarted = Boolean(activeRun?.fieldwork.startedAt);
  ...
  const stages = [
    ...
    { code: "02", zh: "程序测试", en: "Program test", done: programTestDone, active: activeRun?.stage === "program_test", detailZh: "用固定测试入口验证甄别、跳转、必答与完成回传", detailEn: "Use the fixed test URL to validate screening, routing, validation and completion" },
    { code: "03", zh: "DE随机数检查", en: "DE random-seed check", done: softDone, active: activeRun?.stage === "soft_launch", detailZh: "固定随机数并用小流量核对随机组、字段与配额计数", detailEn: "Use a fixed seed and small traffic to check random cells, fields and quota counts" },
    { code: "04", zh: "正式上线回收", en: "Launch fieldwork", done: fieldworkStarted, active: activeRun?.stage === "fieldwork", detailZh: "程序测试和DE均通过后确认正式版本上线", detailEn: "Launch the production version after program and DE checks pass" },
    { code: "05", zh: "配额监控", en: "Quota monitoring", done: fieldworkDone, active: activeRun?.stage === "monitoring", detailZh: "通过FRM监测总体、配额单元与有效完成进度", detailEn: "Use FRM to monitor overall, quota-cell and valid-complete progress" },
    { code: "06", zh: "回收结束", en: "Fieldwork close", done: fieldworkDone, active: false, detailZh: "总体和最低配额单元均达到100%后关闭回收", detailEn: "Close after total and minimum quota cells both reach 100%" },
  ];
  const stageLabel = !activeRun ? tr(locale, "未建立", "Not created") : activeRun.stage === "programming" ? tr(locale, "程序配置", "Programming") : activeRun.stage === "program_test" ? tr(locale, "程序测试", "Program test") : activeRun.stage === "soft_launch" ? tr(locale, "DE随机数检查", "DE random-seed check") : activeRun.stage === "fieldwork" ? tr(locale, "正式上线准备", "Launch preparation") : activeRun.stage === "monitoring" ? tr(locale, "正式回收与配额监控", "Fieldwork and quota monitoring") : activeRun.stage === "closed" ? tr(locale, "等待数据确认", "Waiting for data confirmation") : tr(locale, "生产数据已绑定", "Production data bound");
  ...
  return <div className="snack-stack project-execution-hub client-progress-hub">
    ...
    {activeRun ? <section className="execution-demo-toolbar"><div><span>{tr(locale, "逐步验证", "Step-by-step verification")}</span><strong>{tr(locale, "每一步均可载入案例结果并单独确认，不会跳过下一阶段门槛。", "Each step can load case results and be confirmed separately without bypassing the next gate.")}</strong></div><button type="button" onClick={resetRun}>{tr(locale, "重新走一遍流程", "Restart workflow")}</button></section> : null}
    {activeRun ? <section className="execution-stage-control">
      {activeRun.stage === "programming" ? <><header><span>01 · PROGRAMMING</span><h3>{tr(locale, "确认程序版本与固定测试入口", "Confirm program version and fixed test URL")}</h3></header><div className="execution-control-fields"><label><span>{tr(locale, "程序版本 / ID", "Program version / ID")}</span><input value={programId} onChange={(event) => setProgramId(event.target.value)} /></label><label><span>{tr(locale, "测试访问链接", "Test access URL")}</span><input value={accessUrl} onChange={(event) => setAccessUrl(event.target.value)} /></label></div><footer><p>{tr(locale, "案例已预置可访问的固定测试入口；确认版本后先进入程序测试，不直接进入DE。", "A fixed test URL is prefilled; confirm the version and run program testing before DE.")}</p><button type="button" disabled={!programConfigurationReady(programId, accessUrl)} onClick={saveProgramming}>{tr(locale, "进入程序测试", "Continue to program test")}</button></footer></> : null}
      {activeRun.stage === "program_test" ? <><header><span>02 · PROGRAM TEST</span><h3>{tr(locale, "逐项确认程序测试结果", "Confirm program-test results")}</h3></header><div className="program-test-entry"><div><span>{activeRun.program?.programId}</span><strong>{activeRun.program?.accessUrl}</strong></div><a href={activeRun.program?.accessUrl} target="_blank" rel="noreferrer">{tr(locale, "打开固定测试入口", "Open fixed test URL")} ↗</a></div><div className="program-test-checks">{([{ key: "questionnaireLoaded", zh: "问卷页面与版本", en: "Pages and version" }, { key: "routingPassed", zh: "甄别、跳转与终止", en: "Screening and routing" }, { key: "validationPassed", zh: "必答、互斥与格式", en: "Required and format rules" }, { key: "completionPassed", zh: "完成页与字段回传", en: "Completion and field return" }] as const).map((item) => <label key={item.key} className={programChecks[item.key] ? "checked" : ""}><input type="checkbox" checked={programChecks[item.key]} onChange={(event) => setProgramChecks((current) => ({ ...current, [item.key]: event.target.checked }))} /><span>{tr(locale, item.zh, item.en)}</span></label>)}</div><footer><p>{tr(locale, "四项程序测试均通过后，才进入固定随机数与小流量检查。", "All four program checks must pass before the fixed-seed small-traffic check.")}</p><div><button type="button" className="execution-quick-fill" onClick={() => setProgramChecks({ questionnaireLoaded: true, routingPassed: true, validationPassed: true, completionPassed: true })}>{tr(locale, "载入案例测试结果", "Load case test results")}</button><button type="button" className="primary" disabled={!checkSetReady(programChecks)} onClick={saveProgramTest}>{tr(locale, "确认程序测试", "Confirm program test")}</button></div></footer></> : null}
      {activeRun.stage === "soft_launch" ? <><header><span>03 · DE RANDOM-SEED CHECK</span><h3>{tr(locale, "登记固定随机数与小流量检查结果", "Register the fixed seed and small-traffic checks")}</h3></header><div className="execution-control-fields"><label><span>{tr(locale, "DE随机数", "DE random seed")}</span><input value={randomSeed} onChange={(event) => setRandomSeed(event.target.value)} placeholder={`DE-${activeRun.designVersion}-01`} /></label><label><span>{tr(locale, "有效完成样本", "Valid completes")}</span><input type="number" min="0" value={softLaunchN} onChange={(event) => setSoftLaunchN(event.target.value)} placeholder="30" /></label></div><div className="soft-launch-control"><div>{([{ key: "routingPassed", zh: "跳转与Base", en: "Routing and bases" }, { key: "randomizationPassed", zh: "随机分组平衡", en: "Random-cell balance" }, { key: "fieldMapPassed", zh: "Raw字段映射", en: "Raw field mapping" }, { key: "quotaCountPassed", zh: "配额计数", en: "Quota counts" }] as const).map((item) => <label key={item.key} className={softChecks[item.key] ? "checked" : ""}><input type="checkbox" checked={softChecks[item.key]} onChange={(event) => setSoftChecks((current) => ({ ...current, [item.key]: event.target.checked }))} /><span>{tr(locale, item.zh, item.en)}</span></label>)}</div></div><footer><p>{tr(locale, "固定随机数、N≥30且四项全部通过后进入正式上线准备。", "A fixed seed, N≥30 and all four checks are required before launch preparation.")}</p><div><button type="button" className="execution-quick-fill" onClick={() => { setRandomSeed(`DE-${activeRun.designVersion}-01`); setSoftLaunchN("50"); setSoftChecks({ routingPassed: true, randomizationPassed: true, fieldMapPassed: true, quotaCountPassed: true }); }}>{tr(locale, "载入案例DE结果", "Load case DE results")}</button><button type="button" className="primary" disabled={!deCheckReady(Number(softLaunchN), randomSeed, softChecks)} onClick={saveSoftLaunch}>{tr(locale, "确认DE检查", "Confirm DE check")}</button></div></footer></> : null}
      {activeRun.stage === "fieldwork" ? <><header><span>04 · LAUNCH FIELDWORK</span><h3>{tr(locale, "确认正式版本上线回收", "Launch the production survey")}</h3></header><div className="fieldwork-launch-evidence"><article><span>{tr(locale, "程序版本", "Program version")}</span><strong>{activeRun.program?.programId}</strong></article><article><span>{tr(locale, "程序测试", "Program test")}</span><strong>{activeRun.programTest ? "4 / 4 PASS" : "—"}</strong></article><article><span>{tr(locale, "DE检查", "DE check")}</span><strong>{activeRun.softLaunch ? `N=${activeRun.softLaunch.completedN} · PASS` : "—"}</strong></article></div><footer><p>{tr(locale, "正式上线后开始记录有效完成样本，并进入FRM配额监控。", "After launch, valid completes are recorded and FRM quota monitoring begins.")}</p><button type="button" className="primary" onClick={confirmFieldworkLaunch}>{tr(locale, "确认正式上线", "Confirm launch")}</button></footer></> : null}
      {activeRun.stage === "monitoring" ? <><header><span>05 · FIELDWORK MONITORING</span><h3>{tr(locale, "正式回收与FRM配额监控", "Main fieldwork and FRM quota monitoring")}</h3></header><div className="execution-control-fields"><label><span>{tr(locale, "当前有效完成样本", "Current valid completes")}</span><input type="number" min="0" max={activeRun.targetN} value={completedN} onChange={(event) => setCompletedN(event.target.value)} /></label><label><span>{tr(locale, "最低配额单元完成率", "Minimum quota-cell completion")}</span><input type="number" min="0" max="100" value={minimumQuotaCompletion} onChange={(event) => setMinimumQuotaCompletion(event.target.value)} /></label><label className="execution-frm-field"><span>FRM {tr(locale, "监控链接", "monitoring URL")}</span><input value={frmUrl} onChange={(event) => setFrmUrl(event.target.value)} /></label><a className="execution-frm-link" href={/^https?:\/\//i.test(frmUrl.trim()) ? frmUrl.trim() : defaultFrmUrl} target="_blank" rel="noreferrer">{tr(locale, "打开FRM配额监控", "Open FRM quota monitor")} ↗</a></div><footer><p>{tr(locale, `关闭条件：有效完成N=${activeRun.targetN.toLocaleString()}且最低配额单元达到100%。`, `Close when valid completes reach N=${activeRun.targetN.toLocaleString()} and the minimum quota cell reaches 100%.`)}</p><div><button type="button" className="execution-quick-fill" onClick={() => { setCompletedN(String(activeRun.targetN)); setMinimumQuotaCompletion("100"); }}>{tr(locale, "载入案例完成进度", "Load completed case progress")}</button><button type="button" onClick={() => saveFieldwork(false)}>{tr(locale, "保存进度", "Save progress")}</button><button type="button" className="primary" disabled={!fieldworkCloseReady(Number(completedN), activeRun.targetN, Number(minimumQuotaCompletion))} onClick={() => saveFieldwork(true)}>{tr(locale, "关闭回收", "Close fieldwork")}</button></div></footer></> : null}
      {activeRun.stage === "closed" ? <div className="execution-ready-for-data"><span>06 · FIELDWORK CLOSED</span><h3>{tr(locale, "回收已关闭，进入第04步确认生产数据", "Fieldwork is closed; confirm production data in step 04")}</h3><p>{tr(locale, "可直接使用当前回收结果，或上传另一份已确认Raw Data。", "Use the current collection or upload another confirmed Raw Data file.")}</p></div> : null}
...
function InsightDeliveryHub({ locale, production, run, history, loadState, onSelectRun, onRunChange }: { locale: Locale; production: RawProductionResult | null; run: ProjectRunRecord | null; history: ProjectRunRecord[]; loadState: "idle" | "loading" | "ready" | "error"; onSelectRun: (resultKey: string) => void; onRunChange: (run: ProjectRunRecord) => void }) {
  const [deliveryVersion, setDeliveryVersion] = useState("FINAL-R1");
  ...
  const registerDelivery = () => {
    if (!run || !deliveryFile || !deliveryVersion.trim()) return;
    const extension = deliveryFile.name.split(".").pop()?.toLowerCase() ?? "";
    const kind = ["ppt", "pptx", "doc", "docx", "pdf", "html"].includes(extension) ? "report" : ["xlsx", "xls", "csv"].includes(extension) ? "table" : extension === "json" ? "model" : "other";
    const now = new Date().toISOString();
    onRunChange({ ...run, updatedAt: now, finalDeliverables: [...(run.finalDeliverables ?? []), { fileName: deliveryFile.name, version: deliveryVersion.trim(), kind, size: deliveryFile.size, registeredAt: now }] });
  ...
  return <div className="snack-stack insight-delivery-hub">
    ...
    <section className="final-delivery-register"><header><div><span>FINAL DELIVERABLE</span><h3>{tr(locale, "登记最终交付版本", "Register the final delivery version")}</h3></div><p>{tr(locale, "AI生成稿与正式交付分开管理；最终文件按项目运行版本登记。", "AI-generated drafts and final deliveries are versioned separately against the project run.")}</p></header><div><label><span>{tr(locale, "交付版本", "Delivery version")}</span><input value={deliveryVersion} onChange={(event) => setDeliveryVersion(event.target.value)} /></label><label className="final-delivery-file"><span>{deliveryFile?.name ?? tr(locale, "选择报告、模型附录或Table", "Choose a report, model appendix or table")}</span><input type="file" accept=".ppt,.pptx,.pdf,.doc,.docx,.html,.xlsx,.xls,.csv,.json" onChange={(event) => setDeliveryFile(event.target.files?.[0] ?? null)} /></label><button type="button" disabled={!deliveryFile || !deliveryVersion.trim()} onClick={registerDelivery}>{tr(locale, "确认最终版本", "Confirm final version")}</button></div>{run?.finalDeliverables?.length ? <section>{run.finalDeliverables.map((item) => <article key={`${item.registeredAt}-${item.fileName}`}><span>{item.version}</span><strong>{item.fileName}</strong><small>{item.kind.toUpperCase()} · {(item.size / 1024 / 1024).toFixed(2)} MB · {new Date(item.registeredAt).toLocaleString(locale === "zh" ? "zh-CN" : "en-GB")}</small></article>)}</section> : null}</section>
  </div>;
}
...
export default function SnackIndustryFocusDashboard() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        const validStage = ["programming", "program_test", "soft_launch", "fieldwork", "monitoring", "closed", "data_ready"].includes(parsed.stage);
  ...
  const bindFinalRaw = useCallback((result: RawProductionResult, source: "current_collection" | "uploaded_raw") => {
    ...
    const next: ProjectRunRecord = {
      ...
      stage: "data_ready",
      updatedAt: now,
      dataConfirmation: { source, fileName: result.meta.fileName, confirmedAt: now },
      finalRaw: {
