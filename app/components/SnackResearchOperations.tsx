...
export default function SnackResearchOperations() {
  ...
  return <main className="research-ops-shell">
    ...
    {view === "qc" && <section className="research-ops-content"><header><div><span>RAW DATA QC</span><h2>规则命中、复核与最终纳入</h2></div><a href={publicAssetPath("/downloads/零食消费者研究_Raw_Data_QC-20260817.xlsx")} download>下载QC工作簿</a></header><div className="ops-qc-table"><header><span>Rule</span><span>检查对象</span><span>判定口径</span><span>处理</span></header>{qcRows.map((row) => <div key={row[0]}><b>{row[0]}</b><strong>{row[1]}</strong><p>{row[2]}</p><span>{row[3]}</span></div>)}</div></section>}
