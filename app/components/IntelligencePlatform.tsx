  ...
  return (
    <>
      ...
      <section className="scenario-banner panel">
        <div><span className="eyebrow">RECOMMENDED REALLOCATION</span><h2>Move {shift}% from display into online video and retail proof.</h2><p>Expected incremental consideration increases from 6.8 to {expectedLift} points at the same total budget.</p></div>
        <label className="range-control"><span>Budget shift</span><strong>{shift}%</strong><input type="range" min="0" max="24" value={shift} onChange={(event) => setShift(Number(event.target.value))} /></label>
      </section>
      <section className="two-column">
